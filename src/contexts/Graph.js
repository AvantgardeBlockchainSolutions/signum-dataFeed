...
const Graph = ({ children }) => {
  ...
  const [graphPulsechainData, setGraphPulsechainData] = useState({})
  ...
  //Pulsechain
  const pulsechain = useQuery(reporterQuery, {
    client: clientPulsechain,
    fetchPolicy: 'network-only',
    pollInterval: 5000,
  })
  const pulsePay = useQuery(autopayQuery, {
    client: clientPulsechain,
    fetchPolicy: 'network-only',
    pollInterval: 5000,
  })
...
graphPulsechainData.data.newReportEntities.forEach((event) => {
...
graphZksyncTestData.data.newReportEntities.forEach((event) => {
  ...
  }, [graphMainnetData, graphPulsechainData, graphSepoliaData, graphMaticData, graphAmoyData, graphArboneData, graphGnosismainData, graphOptMainData, graphOptTestData, graphLineaData, graphLineaTestData, graphPolygonzkData, graphZksyncMainData, graphZksyncTestData]);
